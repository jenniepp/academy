...
public class preImpl implements pre{//3. 인터페이스구현 및 오버라이드해서 내용 만들기
	...
	public void searchingMember() {
		...
		String std=sc.next();
		List<preVO> lists2 = new ArrayList<>();
		
		Iterator<preVO> it=lists.iterator();		//이터레이터
		Iterator<preVO> it2=lists2.iterator();	
		preVO vo2=new preVO();
		while(it.hasNext()) {
			preVO vo=it.next();
			if(std.equals(vo.getName())) {
				...
				vo2.setMemberCode(vo.getMemberCode());
				vo2.setBirth(vo.getBirth());
				vo2.setLastDate(vo.getLastDate());
				vo2.setPoint(vo.getPoint());
				
				lists.add(vo2);
				
				/*if(prelists.size()>=2) {
					System.out.println("결과가 2건 이상입니다. 추가적으로 생년월일을 입력해주세요");
					String prestd=sc.next();
					if(prestd.equals(prevo.getBirth())) {
						System.out.println(prevo.toString());
